"""
retrieve.py — Retrieval pipeline.

retrieve(user_id, query) → ranked list of SnapshotNode result dicts.

Flow:
  1. Decompose long queries via decomposer; embed each segment in parallel.
  2. Wide k-NN against SnapshotNode centroids; scores merged by similarity.
  3. Ensure snapshot summaries are valid (lazy LLM if needed).
  4. Rerank candidates with reranker; keep top_k.
  5. For each result: traverse SYNTHESIZES → Episode members (ordered by timestamp).
  6. Translate confidence floats to natural-language labels.
"""

import asyncio

from . import config
from .chain import ensure_snapshot_summary
from .db import get_graph, init_graph
from .embed import embed
from .decomposer import decompose
from .rerank import rerank

_LONG_QUERY_WORDS = 30
_TOP_K = 5


# ---------------------------------------------------------------------------
# Confidence translation
# ---------------------------------------------------------------------------


def _confidence_label(confidence: float, episodic: bool = False) -> str:
    if episodic:
        return "vivid, unverified by convergence"
    if confidence > 0.75:
        return "established"
    if confidence >= 0.50:
        return "likely"
    if confidence >= 0.30:
        return "uncertain"
    return "contested"


# ---------------------------------------------------------------------------
# ANN on SnapshotNode
# ---------------------------------------------------------------------------


async def _ann_candidates(
    g, query_vec: list[float], k: int
) -> list[tuple[dict, float]]:
    """k-NN against SnapshotNode.centroid. Returns [(snap_dict, score)] descending."""
    try:
        result = await g.query(
            f"CALL db.idx.vector.queryNodes('SnapshotNode', 'centroid', {k}, vecf32($vec)) "
            "YIELD node, score "
            "RETURN node.id, node.summary, node.valid, score",
            {"vec": query_vec},
        )
    except Exception:
        # Index missing (uninitialized graph) — ensure it exists, return empty.
        await init_graph(g.name.removeprefix("bubble:"))
        return []
    out = []
    for row in result.result_set:
        snap = {
            "id": row[0],
            "summary": row[1],
            "valid": bool(row[2]),
        }
        out.append((snap, float(row[3])))
    return out


# ---------------------------------------------------------------------------
# Traversal: SnapshotNode → members → edge context
# ---------------------------------------------------------------------------


async def _get_members(g, snap_id: str) -> list[dict]:
    """Return Episode members belonging to this SnapshotNode, ordered by timestamp."""
    result = await g.query(
        "MATCH (snap:SnapshotNode {id: $id})-[:SYNTHESIZES]->(t:Episode) "
        "RETURN t.id, t.summary, t.confidence, t.episodic, t.timestamp "
        "ORDER BY t.timestamp",
        {"id": snap_id},
    )
    return [
        {
            "id": row[0],
            "summary": row[1],
            "confidence": float(row[2]) if row[2] is not None else 0.0,
            "episodic": bool(row[3]),
        }
        for row in result.result_set
    ]


# ---------------------------------------------------------------------------
# Public API
# ---------------------------------------------------------------------------


async def _retrieve_from_vecs(
    g,
    query: str,
    query_vecs: list[list[float]],
    top_k: int,
    verbose: bool = False,
) -> list[dict]:
    """
    Core retrieval given pre-computed query vectors.
    ANN on SnapshotNode → lazy summary generation → optional rerank → traversal → confidence labels.

    verbose=False: returns lightweight list of snapshot dicts (id, summary).
    verbose=True:  includes episode chain members and confidence labels.
    """
    results_per_vec = await asyncio.gather(
        *[_ann_candidates(g, qvec, top_k) for qvec in query_vecs]
    )

    score_map: dict[str, float] = {}
    snap_map: dict[str, dict] = {}
    for candidates in results_per_vec:
        for snap, sim in candidates:
            nid = snap["id"]
            score_map[nid] = score_map.get(nid, 0.0) + sim
            snap_map.setdefault(nid, snap)

    if not score_map:
        return []

    candidate_ids = sorted(score_map, key=lambda nid: score_map[nid], reverse=True)
    candidate_snaps = [snap_map[nid] for nid in candidate_ids]

    # Ensure summaries are valid before reranking (lazy LLM generation).
    summaries = await asyncio.gather(
        *[
            ensure_snapshot_summary(
                g, snap["id"], valid=snap["valid"], summary=snap["summary"]
            )
            for snap in candidate_snaps
        ]
    )
    for snap, summary in zip(candidate_snaps, summaries):
        snap["summary"] = summary or ""

    # Rerank or fall back to ANN score ordering, then trim to top_k.
    if config.RERANK_ENABLED:
        rerank_scores = await rerank(query, [s["summary"] for s in candidate_snaps])
        ranked = sorted(
            zip(candidate_snaps, rerank_scores),
            key=lambda pair: pair[1],
            reverse=True,
        )[:top_k]
    else:
        ranked = [(s, score_map[s["id"]]) for s in candidate_snaps[:top_k]]

    if not verbose:
        return [{"id": snap["id"], "summary": snap["summary"]} for snap, _ in ranked]

    # Verbose: enrich each result with episode chain members + confidence labels.
    traversals = await asyncio.gather(
        *[_get_members(g, snap["id"]) for snap, _ in ranked]
    )
    return [
        {
            "id": snap["id"],
            "summary": snap["summary"],
            "members": [
                {
                    "id": m["id"],
                    "summary": m["summary"],
                    "confidence_label": _confidence_label(m["confidence"], m["episodic"]),
                }
                for m in members
            ],
            "context": [],
        }
        for (snap, _), members in zip(ranked, traversals)
    ]


async def retrieve(user_id: str, query: str, top_k: int = _TOP_K, verbose: bool = False) -> list[dict]:
    """
    Retrieve relevant SnapshotNodes for a query.

    verbose=False (default): returns [{id, summary}, ...] — fast, no chain traversal.
    verbose=True:            returns [{id, summary, members, context}, ...].
    """
    g = get_graph(user_id)

    words = query.split()
    if len(words) > _LONG_QUERY_WORDS:
        segments = await decompose(query)
        query_vecs = (
            list(await asyncio.gather(*[embed(s["text"]) for s in segments]))
            if segments
            else [await embed(query)]
        )
    else:
        query_vecs = [await embed(query)]

    return await _retrieve_from_vecs(g, query, query_vecs, top_k, verbose)
